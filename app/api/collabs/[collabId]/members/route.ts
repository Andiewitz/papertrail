...
export async function GET(_: Request, { params }: { params: Promise<{ collabId: string }> }) {
  try {
    ...
    const result = await (await db()).execute({ sql: "SELECT users.id, users.email, collab_memberships.role, collab_memberships.status, collab_memberships.created_at FROM collab_memberships JOIN users ON users.id = collab_memberships.user_id WHERE collab_memberships.collab_id = ? ORDER BY CASE collab_memberships.role WHEN 'admin' THEN 0 WHEN 'co_admin' THEN 1 ELSE 2 END, users.email", args: [collabId] });
    return NextResponse.json({ members: result.rows.map((row) => ({ id: String(row.id), email: String(row.email), role: String(row.role), status: String(row.status), joinedAt: Number(row.created_at) })) });
