...
async function allEntries(client: Client, userId: string) {
  const result = await client.execute({ sql: "SELECT id, clock_in, clock_out FROM time_entries WHERE user_id = ? ORDER BY clock_in DESC LIMIT 90", args: [userId] });
...
export async function POST(request: Request) {
  try {
    ...
    const active = await client.execute({ sql: "SELECT id FROM time_entries WHERE user_id = ? AND clock_out IS NULL LIMIT 1", args: [user.id] });
    if (!active.rows[0]) {
      if (input.data.action === "clock-out" && input.data.entryId) {
        const completed = await client.execute({ sql: "SELECT id FROM time_entries WHERE id = ? AND user_id = ? AND clock_out IS NOT NULL", args: [input.data.entryId, user.id] });
