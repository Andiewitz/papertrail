...
export async function GET() {
  try {
    ...
    logError("notes_list_failed", error);
    return NextResponse.json({ error: "We couldn't load your notes. Please try again.", code: "NOTES_LOAD_FAILED" }, { status: 500 });
  }
}
...
export async function POST(request: Request) {
  try {
    ...
    logError("note_create_failed", error);
    return NextResponse.json({ error: "We couldn't save your note. Please try again.", code: "NOTE_CREATE_FAILED" }, { status: 500 });
  }
}
